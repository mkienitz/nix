...
if [[ -z "$selected" ]]; then
  gum style "No .direnv directories selected"
  exit 0
fi
...
gum confirm "Run garbage collection now? [requires sudo]" \
  && sudo -v \
  && gum spin --show-stdout --title $'Collecting garbage (sudo)...' -- sudo nix-collect-garbage -d --quiet \
  | gum style --foreground 157 \
  && gum spin --show-stdout --title "Collecting garbage (user)..." -- nix-collect-garbage -d --quiet \
  | gum style --foreground 157 \
  && gum style --foreground 212 --padding "1 2" "✓ Garbage collected"

gum confirm "Run store optimisation now? [requires sudo]" \
  && sudo -v \
  && gum spin --show-stdout --title "Optimising store (sudo)..." -- sudo nix store optimise \
  && gum spin --show-stdout --title "Optimising store (user)..." -- nix store optimise \
  && gum style --foreground 212 --padding "1 2" "✓ Store optimised"

gum style --foreground 212 --padding "1 2" "✓ Cleanup finished"
